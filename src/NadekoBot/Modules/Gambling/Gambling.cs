    ...
    public partial class Gambling : DiscordModule
    {
                ...
                if (System.IO.File.Exists(coloredNamesFile))
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        if (expireDate < System.DateTime.Now)
                        {
                            try
                            {
                                IGuild guild = NadekoBot.Client.Guilds.First();
                                IGuildUser user = NadekoBot.Client.Guilds.First().GetUser(System.Convert.ToUInt64(userId));
                                if (user != null)
                                {
        ...
        public async Task Shop(params string[] args) //string item, string shopparams, [Remainder] string extraparams = null
        {
            ...
            if (args[0] == "sticker")
            {
                ...
                if (expired) {
                    await Context.Channel.SendErrorAsync("Woops! Sorry about that, " + Context.User.Mention + "! Stickers are no longer available to be purchased in the shop, as the sticker event has expired! Keep your eye out for the next sticker event!").ConfigureAwait(false);
                    return;
                }
